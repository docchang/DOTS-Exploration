...
namespace Rival.Samples.OnlineFPS
{
    ...
    public partial class RailgunSystem : SystemBase
    {
        ...
        public AfterGhostPredictionCommandBufferSystem AfterGhostPredictionCommandBufferSystem;
        ...
        protected override void OnCreate()
        {
            ...
            AfterGhostPredictionCommandBufferSystem = World.GetOrCreateSystem<AfterGhostPredictionCommandBufferSystem>();
        ...
        protected override void OnUpdate()
        {
            ...
            EntityCommandBuffer commandBuffer = AfterGhostPredictionCommandBufferSystem.CreateCommandBuffer();
                    ...
                    in PredictedGhostComponent predictedGhost) =>
                {
                    if (GhostPredictionSystemGroup.ShouldPredict(tick, predictedGhost))
                    {
                        ...
                        if (weapon.ShootRequested && railgun._firingTimer <= 0f)
                        {
                            ...
                            // Shot visuals
                            // Only ask for shot visuals if we didn't already spawn shot visuals at that tick
                            if (!isServer && tick > railgun._lastTickShot)
                            {
                                // Spawn hit vfx
                                ...
                                // Handle recoil
                                if (HasComponent<OnlineFPSCharacterComponent>(weapon.OwnerEntity))
                                {
                                    OnlineFPSCharacterComponent owningCharacter = GetComponent<OnlineFPSCharacterComponent>(weapon.OwnerEntity);
            ...
            AfterGhostPredictionCommandBufferSystem.AddJobHandleForProducer(Dependency);
        }
    }
}