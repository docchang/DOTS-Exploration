...
namespace Rival.Samples.OnlineFPS
{
    ...
    {
        public static bool GetGhostPrefabOfType<T>(EntityManager entityManager, Entity ghostCollectionEntity, out Entity prefabEntity) where T : struct
        {
            prefabEntity = default;

            DynamicBuffer<GhostPrefabsReference> prefabs = entityManager.GetBuffer<GhostPrefabsReference>(ghostCollectionEntity);
