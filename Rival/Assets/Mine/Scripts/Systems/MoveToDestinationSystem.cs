using Unity.Burst;
using Unity.Collections;
using Unity.Entities;
using Unity.Jobs;
using Unity.Mathematics;
using Unity.Transforms;

public partial class MoveToDestinationSystem : SystemBase
{
    protected override void OnUpdate()
    {
		float deltaTime = Time.DeltaTime;

        Entities.ForEach((ref Translation translation, ref Rotation rotation, in Destinaion destinaion, in MovementSpeed speed) =>
        {
            if (math.all(destinaion.Value == translation.Value))
			{
                return;
			}

            float3 toDestination = destinaion.Value - translation.Value;

            rotation.Value = quaternion.LookRotation(toDestination, new float3(0, 1, 0));

            float3 movement = math.normalize(toDestination) * speed.Value * deltaTime;

            if (math.length(movement) > math.length(toDestination))
            {
                translation.Value = destinaion.Value;
            }
            else
            {
                translation.Value += movement;
            }
        }).ScheduleParallel();
    }
}
